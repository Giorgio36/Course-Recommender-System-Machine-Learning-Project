...
def train(model_name, params):
    ...
    if model_name == backend.models[0]:
        ...
        with st.spinner('Training...'):
            time.sleep(0.5)
            trained = backend.train(model_name,params)
        st.success('Done!')
...
def predict2(model_name, user_ids, params):
    ...
    with st.spinner('Generating course recommendations: '):
        time.sleep(0.5)
        res = backend.predict2(model_name, user_ids, params)
    st.success('Recommendations generated!')
    return res
...
# training_button = st.checkbox('Train Model')
# training_text = st.sidebar.text('')
# # Start training process
# if training_button:
#     mtrained = train(model_selection, params)
#     m = [mtrained]

# Prediction
st.sidebar.subheader('3. Training & Prediction')
...
if pred_button and selected_courses_df.shape[0] > 0:
    ...
    if (model_selection == backend.models[0] or model_selection == backend.models[4]):
        new_id = backend.add_new_ratings(selected_courses_df['COURSE_ID'].values)

    else:
        new_id = backend.add_new_user_profile(selected_courses_df['COURSE_ID'].values)

    user_ids = [new_id]

    if model_selection == backend.models[4]:
        res_df = predict2(model_selection, user_ids, params)
    else:
